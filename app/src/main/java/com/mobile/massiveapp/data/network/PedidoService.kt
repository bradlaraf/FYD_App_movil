...
class PedidoService @Inject constructor(
    ...
    suspend fun getReporteProductosFrecuentes(
        cardCode: String,
        timeout: Long = 60L,
    ): List<ProductoFrecuente> {
        return withContext(Dispatchers.IO) {
            val configInfo = getConfigInfoForCalls()
            val retrofit = createRetrofitClient(timeout)
            val apiService = retrofit.create(ReportesClient::class.java)
            val requestBody = getXmlParaReporteEstadoCuenta("ReporteProductosFrecuentes", configInfo.configuracion, configInfo.usuario, cardCode)
            val response = apiService.getReporteProductosFrecuentes(configInfo.url, requestBody)
            val result = response.body()?.body?.response?.result
            val json = result?.json
            val gson = Gson()
            val listType = object : TypeToken<List<ProductoFrecuente>>() {}.type
