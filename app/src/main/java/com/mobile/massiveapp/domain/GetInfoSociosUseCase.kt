...
class GetInfoSociosUseCase @Inject constructor(
    ...
    suspend operator fun invoke(progressCallBack: (Int, String, Int) -> Unit): DoError =
        try {
            ...
            infoTableDao.deleteAllTables(listOf("SocioNegocio", "SocioDirecciones", "SocioContactos"))
            datosMaestrosRepository.getDatosMaestrosFromEndpointAndSave(
                listOf(ManagerInputData.CLIENTE_SOCIOS, ManagerInputData.CLIENTE_SOCIOS_DIRECCIONES, ManagerInputData.CLIENTE_SOCIOS_CONTACTOS),
                configuracion,
                usuario,
                url,
                0
