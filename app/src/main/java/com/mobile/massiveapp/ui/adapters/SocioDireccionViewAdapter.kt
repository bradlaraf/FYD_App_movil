...
import com.mobile.massiveapp.ui.view.util.diffutil.DireccionViewDiffUtil
import com.mobile.massiveapp.ui.view.util.diffutil.DireccionesDiffUtil

class SocioDireccionViewAdapter(
    private var dataSet: List<DoDireccionView>,
    private val onClickListener: (DoDireccionView) -> Unit
): RecyclerView.Adapter<SocioDireccionViewAdapter.ViewHolder>() {
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun render(direccion: DoDireccionView, onClickListener: (DoDireccionView) -> Unit){
            clContainer.setOnClickListener { onClickListener(direccion) }
        }
    }

    fun updateList(newDataSet: List<DoDireccionView>){
        val DireccionesDiff = DireccionViewDiffUtil(dataSet, newDataSet)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.txvTitle.text = "${direccion.Direccion}"
        holder.txvOpcion.text = "${direccion.Provincia} - ${direccion.Distrito} - ${direccion.Calle}"
